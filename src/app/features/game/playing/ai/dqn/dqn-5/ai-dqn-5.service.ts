...
export class AiDqn5Service {
  ...
  public static readonly DQN_SETTINGS = {
    files: {
      camaxtli: {
        model: 'divine-light-dqn5-model-camaxtli_7x6',
        progress: 'divine-light-dqn5-progress-camaxtli_7x6',
        loss: 'divine-light-dqn5-loss-camaxtli_7x6'
      },
      nanahuatzin: {
        model: 'divine-light-dqn5-model-nanahuatzin_7x6',
        progress: 'divine-light-dqn5-progress-nanahuatzin_7x6',
        loss: 'divine-light-dqn5-loss-nanahuatzin_7x6'
      },
    },
    opponent: 'hybrid', // 'random' | 'minimax' | 'hybrid'
  };
  ...
  private run(state: number[][], isTraining: GodType, rounds: number): void {
    ...
    if (trainState.episode >= trainState.totalEpisodes) {
      this.aiDqnTrainService.stop("DQN-5");
      return;
    }
  ...
  private async fit(isTraining: GodType): Promise<void> {
    ...
    const qOnlineValuesFromNextStates: number[][] = isTraining === GodType.CAMAXTLI ?
      this.aiTensorflowService.predictBitmapWithBatch(this.onlineCamaxtli, nextStateList) : this.aiTensorflowService.predictBitmapWithBatch(this.onlineNanahuatzin, nextStateList);
    ...
    for (let j = 0; j < miniBatchSamples.length; j++) {
      ...
      if (!entry.done) {
        ...
        // Double DQN
        const argMaxQOnlineNextState: number = this.aiDqnTrainService.getQValueMaxFromStateWithPrediction(
          entry.nextState, qOnlineValuesFromNextStates[j], isTraining
        );
        const a: number = qOnlineValuesFromNextStates[j].indexOf(argMaxQOnlineNextState);
        const x: number = qValuesFromNextStates[j][a];
  ...
  downloadLoss(isTraining: GodType): void {
    ...
    this.aiDqnTrainService.downloadLoss(isTraining === GodType.CAMAXTLI ?
      AiDqn5Service.DQN_SETTINGS.files.camaxtli.loss + '_' + count : AiDqn5Service.DQN_SETTINGS.files.nanahuatzin.loss + '_' + count
    );
  }
  ...
  downloadProgress(isTraining: GodType): void {
    ...
    this.aiDqnTrainService.downloadProgress(isTraining === GodType.CAMAXTLI ?
      AiDqn5Service.DQN_SETTINGS.files.camaxtli.progress + '_' + count : AiDqn5Service.DQN_SETTINGS.files.nanahuatzin.progress + '_' + count
    );
  }
