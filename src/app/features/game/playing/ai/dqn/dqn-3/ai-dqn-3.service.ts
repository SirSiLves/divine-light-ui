...
export class AiDqn3Service {

  public static readonly DQN_SETTINGS = {
    files: {
      camaxtli: {
        model: 'divine-light-dqn3-model-camaxtli_7x6',
        progress: 'divine-light-dqn3-progress-camaxtli_7x6',
        loss: 'divine-light-dqn3-loss-camaxtli_7x6'
      },
      nanahuatzin: {
        model: 'divine-light-dqn3-model-nanahuatzin_7x6',
        progress: 'divine-light-dqn3-progress-nanahuatzin_7x6',
        loss: 'divine-light-dqn3-loss-nanahuatzin_7x6'
      },
    },
  };
  ...
  private run(state: number[][], isTraining: GodType, rounds: number): void {
    ...
    if (trainState.episode >= trainState.totalEpisodes) {
      this.aiDqnTrainService.stop("DQN-3");
      return;
    }
    ...
    if (this.replayMemory.length >= AiDqnService.ALL_DQN_SETTINGS.replayStartSize) {
      ...
      this.fit(isTraining).then(() => {
        ...
        // 6. go further until batch size is reached
  ...
  downloadLoss(isTraining: GodType): void {
    ...
    this.aiDqnTrainService.downloadLoss(isTraining === GodType.CAMAXTLI ?
      AiDqn3Service.DQN_SETTINGS.files.camaxtli.loss + '_' + count : AiDqn3Service.DQN_SETTINGS.files.nanahuatzin.loss + '_' + count
    );
  }
  ...
  downloadProgress(isTraining: GodType): void {
    ...
    this.aiDqnTrainService.downloadProgress(isTraining === GodType.CAMAXTLI ?
      AiDqn3Service.DQN_SETTINGS.files.camaxtli.progress + '_' + count : AiDqn3Service.DQN_SETTINGS.files.nanahuatzin.progress + '_' + count
    );
  }
